...
def gram_loss_rotate(input: torch.Tensor, target, weight: float = 1.0):
    """Computes MSE Loss for 2 Gram matrices of the same type.
    ...
    Returns
        loss (torch.Tensor): computed loss value.
    """
    Bi, Bt = input.size(0), target[0].size(0)
    assert Bi == Bt

    Ni, Nt = input.size(1), target[0].size(1)
    assert Ni == Nt

    Mi, Mt = input.size(-2) * input.size(-1), target[0].size(-2) * target[0].size(-1)
    assert Mi == Mt
